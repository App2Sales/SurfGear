...
class _VideoSubtitleState extends State<VideoSubtitle> {
  ...
  void didUpdateWidget(VideoSubtitle oldWidget) {
    ...
    if (oldWidget.url != widget.url) {
      _initListeners(fromUpdate: true);
      // _getSubtitles().then((subtitles) {
      //   _subtitles = subtitles;
      //   setState(() {});
      // });
  ...
  void _initListeners({bool fromUpdate = false}) async {
    _subtitles = await _getSubtitles();

    setState(() {});

    if (fromUpdate) {
      _subtitle = _findSubtitle();
      setState(() {});
    }
  ...
  void _scheduleNextSubtitle() {
    ...
    if (_isPlaying) {
      ...
      if (nextSubtitle != null) {
        ...
        if (_subtitles != null && _subtitles.isNotEmpty) {
          _openSubtitle(
              Subtitle()..range = Range(0, _subtitles[0].range.begin));
        } else {
          _openSubtitle(Subtitle()..range = Range(0, 1));
        }
      }
    }
  }
